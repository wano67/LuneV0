...
const shared_expense_service_1 = require("@/modules/shared-expense/shared-expense.service");
async function main() {
    ...
    const expense = await shared_expense_service_1.sharedExpenseService.createSharedExpense({
        userId,
        label: 'Weekend trip',
        totalAmount: 100,
        currency: 'EUR',
    ...
    const { balances: balancesBefore } = await shared_expense_service_1.sharedExpenseService.listSharedExpensesWithBalances(userId);
    ...
    await shared_expense_service_1.sharedExpenseService.settleDebt({
        userId,
    ...
    const { balances: balancesAfter } = await shared_expense_service_1.sharedExpenseService.listSharedExpensesWithBalances(userId);

...
async function main() {
    ...
    await budget_service_1.budgetService.createPersonalBudget({
        userId,
        name: `Budget ${ts}`,
        periodType: 'monthly',
        year,
    ...
    await prisma_1.prisma.business_settings.update({
    ...
    await prisma_1.prisma.invoices.create({
        data: {
            business_id: businessId,
            client_id: client.id,
            project_id: null,
    ...
    const personalInsights = await insights_service_1.insightsService.computePersonalInsights(userId, { year, month });
    const businessInsights = await insights_service_1.insightsService.computeBusinessInsights(userId, businessId, { year, month });
    ...
    const expectBusiness = businessInsights.some((i) => i.id === 'business-late-invoices' || i.id === 'business-under-target-revenue');
    if (!expectPersonal)
        throw new Error('Expected personal-budget-overspent insight');
    if (!expectBusiness)
        throw new Error('Expected business insights for late invoices or under target revenue');

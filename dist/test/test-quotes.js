...
async function main() {
    ...
    // 5) CREATE QUOTE
    //
    const quote = await quote_service_1.quotesService.createQuote({
        userId: user.id,
        businessId,
        clientId: client.id,
        currency: 'EUR',
    ...
    // 6) ACCEPT QUOTE
    //
    const accepted = await quote_service_1.quotesService.updateQuoteStatus(quote.id, user.id, 'accepted');
    ...
    //
    const payment = await invoice_payment_service_1.invoicePaymentService.registerInvoicePayment({
        userId: user.id,
        businessId,

...
const transaction_import_service_1 = require("@/modules/transaction/transaction-import.service");
...
async function main() {
    console.log('🔹 Starting domain smoke test (User + Business + Account + Transaction + Budget + Savings + Cashflow + Insights)...');
    const timestamp = Date.now();
    const email = `test.user+${timestamp}@example.com`;
    // --- 1) createUserWithDefaultSettings ---
    console.log('\n=== 1) createUserWithDefaultSettings ===');
    const createdUser = await user_service_1.userService.createUserWithDefaultSettings({
        email,
    ...
    console.log('\n=== 2) getUserWithSettingsById ===');
    const userWithSettings = await user_service_1.userService.getUserWithSettingsById(userId);
    ...
    console.log('\n=== 3) createUserWithDefaultSettings (duplicate email) ===');
    try {
        await user_service_1.userService.createUserWithDefaultSettings({
            email,
    ...
    console.log('\n=== 4) createBusinessWithDefaultSettings ===');
    const createdBusiness = await business_service_1.businessService.createBusinessWithDefaultSettings({
        userId,
    ...
    console.log('\n=== 5) listBusinessesForUser ===');
    const businessesForUser = await business_service_1.businessService.listBusinessesForUser(userId);
    ...
    console.log('\n=== 6) updateBusinessProfile ===');
    const updatedBusiness = await business_service_1.businessService.updateBusinessProfile(businessId, userId, {
    ...
    console.log('\n=== 7) updateBusinessSettings ===');
    const updatedBusinessSettings = await business_service_1.businessService.updateBusinessSettings(businessId, userId, {
    ...
    console.log('\n=== 8) archiveBusiness / reactivateBusiness ===');
    const archivedBusiness = await business_service_1.businessService.archiveBusiness(businessId, userId);
    console.log('✅ Business archived, is_active:', archivedBusiness.business.is_active);
    const reactivatedBusiness = await business_service_1.businessService.reactivateBusiness(businessId, userId);
    console.log('✅ Business reactivated, is_active:', reactivatedBusiness.business.is_active);
    // --- 9) BusinessOwnershipError (fake user) ---
    console.log('\n=== 9) BusinessOwnershipError (fake user) ===');
    try {
        await business_service_1.businessService.updateBusinessProfile(businessId, userId + 1000n, { name: 'Hacked Name' });
    ...
    console.log('\n=== 10) createPersonalAccount ===');
    const personalAccount = await account_service_1.accountService.createPersonalAccount({
        userId,
    ...
    console.log('\n=== 11) createBusinessAccount ===');
    const businessAccount = await account_service_1.accountService.createBusinessAccount({
        userId,
        businessId,
    ...
    console.log('\n=== 12) listPersonalAccountsForUser ===');
    const personalAccounts = await account_service_1.accountService.listPersonalAccountsForUser(userId, {});
    ...
    console.log('\n=== 13) listBusinessAccountsForUser ===');
    const businessAccounts = await account_service_1.accountService.listBusinessAccountsForUser(userId, businessId, {});
    ...
    console.log('\n=== 14) updateAccount (personal account) ===');
    const updatedPersonalAccount = await account_service_1.accountService.updateAccount(personalAccountId, userId, {
    ...
    console.log('\n=== 15) archiveAccount / reactivateAccount (business account) ===');
    const archivedAccount = await account_service_1.accountService.archiveAccount(businessAccountId, userId);
    console.log('✅ Business account archived, is_active:', archivedAccount.is_active);
    const reactivatedAccount = await account_service_1.accountService.reactivateAccount(businessAccountId, userId);
    console.log('✅ Business account reactivated, is_active:', reactivatedAccount.is_active);
    // --- 16) AccountOwnershipError (fake user) ---
    console.log('\n=== 16) AccountOwnershipError (fake user) ===');
    try {
        await account_service_1.accountService.getAccountForUser(personalAccountId, userId + 1000n);
    ...
    console.log('\n=== 17) seed categories ===');
    const incomeCategory = await prisma_1.prisma.categories.create({
        data: {
            user_id: userId,
    ...
    const expenseCategory = await prisma_1.prisma.categories.create({
        data: {
            user_id: userId,
    ...
    console.log('\n=== 18) createPersonalTransaction ===');
    const personalTx = await transaction_service_1.transactionService.createPersonalTransaction({
        userId,
    ...
    console.log('\n=== 19) createBusinessTransaction ===');
    const businessTx = await transaction_service_1.transactionService.createBusinessTransaction({
        userId,
        businessId,
    ...
    // --- 20) createTransfer ---
    console.log('\n=== 20) createTransfer ===');
    const transfer = await transaction_service_1.transactionService.createTransfer({
        userId,
    ...
    console.log('\n=== 21) listTransactionsForAccount (personal) ===');
    const personalTxs = await transaction_service_1.transactionService.listTransactionsForAccount(userId, personalAccountId, {});
    ...
    console.log('\n=== 22) listTransactionsForUser (personal only) ===');
    const personalOnlyTxs = await transaction_service_1.transactionService.listTransactionsForUser(userId, { businessId: null });
    ...
    console.log('\n=== 23) listTransactionsForUser (business only) ===');
    const businessOnlyTxs = await transaction_service_1.transactionService.listTransactionsForUser(userId, { businessId });
    ...
    console.log('\n=== 24) updateTransaction ===');
    const updatedTx = await transaction_service_1.transactionService.updateTransaction(personalTx.id, userId, {
    ...
    console.log('\n=== 25) deleteTransactionForUser ===');
    await transaction_service_1.transactionService.deleteTransactionForUser(businessTx.id, userId);
    console.log('✅ Business transaction deleted');
    try {
        await transaction_service_1.transactionService.getTransactionForUser(businessTx.id, userId);
    ...
    console.log('\n=== 26) TransactionOwnershipError (fake user) ===');
    try {
        await transaction_service_1.transactionService.getTransactionForUser(personalTx.id, userId + 1000n);
    ...
    console.log('\n=== 27) createPersonalBudget + lines ===');
    const now = new Date();
    const personalBudget = await budget_service_1.budgetService.createPersonalBudget({
        userId,
    ...
    const incomeLine = await budget_service_1.budgetService.addBudgetLine({
        userId,
    ...
    const expenseLine = await budget_service_1.budgetService.addBudgetLine({
        userId,
        budgetId: personalBudget.budget.id,
        categoryId: expenseCategory.id,
        spendingLimit: 800,
    ...
    console.log('\n=== 28) computeBudgetExecutionForUser ===');
    const execution = await budget_service_1.budgetService.computeBudgetExecutionForUser(personalBudget.budget.id, userId);
    ...
    console.log('\n=== 29) updateBudgetLine + deleteBudgetLineForUser ===');
    const updatedIncomeLine = await budget_service_1.budgetService.updateBudgetLine(incomeLine.id, userId, {
        spendingLimit: 2100,
    ...
    await budget_service_1.budgetService.deleteBudgetLineForUser(expenseLine.id, userId);
    console.log('✅ Budget line deleted');
    // --- 30) BudgetOwnershipError (fake user) ---
    console.log('\n=== 30) BudgetOwnershipError (fake user) ===');
    try {
        await budget_service_1.budgetService.getBudgetWithLinesForUser(personalBudget.budget.id, userId + 1000n);
    ...
    console.log('\n=== 31) createPersonalSavingsGoal ===');
    const personalGoal = await savings_service_1.savingsService.createPersonalSavingsGoal({
        userId,
    ...
    console.log('\n=== 32) createBusinessSavingsGoal ===');
    const businessGoal = await savings_service_1.savingsService.createBusinessSavingsGoal({
        userId,
        businessId,
    ...
    console.log('\n=== 33) listSavingsGoalsForUser (personal vs business) ===');
    const personalGoals = await savings_service_1.savingsService.listSavingsGoalsForUser(userId, { businessId: null });
    const businessGoals = await savings_service_1.savingsService.listSavingsGoalsForUser(userId, { businessId });
    ...
    console.log('\n=== 34) updateSavingsGoal + computeSavingsOverviewForUser ===');
    const updatedPersonalGoal = await savings_service_1.savingsService.updateSavingsGoal(personalGoal.id, userId, {
        currentAmount: 1500,
    ...
    const savingsOverview = await savings_service_1.savingsService.computeSavingsOverviewForUser(userId, {});
    ...
    console.log('\n=== 35) SavingsGoalOwnershipError (fake user) ===');
    try {
        await savings_service_1.savingsService.getSavingsGoalForUser(personalGoal.id, userId + 1000n);
...
${now.toISOString().slice(0, 10)};Remboursement;200.00`;
    const importResult = await transaction_import_service_1.transactionImportService.importPersonalTransactionsFromCsv({
        userId,
        accountId: personalAccountId,
        csvContent,
    ...
    console.log('\n=== 38) listPersonalAccountsWithBalanceForUser ===');
    const accountsWithBalance = await account_service_1.accountService.listPersonalAccountsWithBalanceForUser(userId);
    console.log('✅ Personal accounts with balance:', accountsWithBalance.map((a) => ({ id: a.account.id, balance: a.balance })));
    // --- 39) computeCurrentPersonalBudgetOverview ---
    console.log('\n=== 39) computeCurrentPersonalBudgetOverview ===');
    const budgetOverview = await budget_service_1.budgetService.computeCurrentPersonalBudgetOverview(userId, {});
    if (budgetOverview) {
    ...
    console.log('\n=== 40) computePersonalSavingsOverview ===');
    const savingsOverviewPersonal = await savings_service_1.savingsService.computePersonalSavingsOverview(userId);
    ...
    console.log('\n=== 41) computePersonalCashflowProjection ===');
    const personalCashflow = await cashflow_service_1.cashflowService.computePersonalCashflowProjection(userId, {
        horizonDays: 90,
    });
    ...
    console.log('\n=== 42) computePersonalInsights ===');
    const personalInsights = await insights_service_1.insightsService.computePersonalInsights(userId);
    ...
    console.log('\n=== 43) create project client ===');
    const projectClient = await client_service_1.clientService.createClient({
        userId,
        businessId,
    ...
    console.log('\n=== 44) create service catalog entries ===');
    const brandingService = await service_service_1.servicesService.createService({
        userId,
        businessId,
    ...
    const devDayService = await service_service_1.servicesService.createService({
        userId,
        businessId,
    ...
    console.log('\n=== 45) create project with services ===');
    const createdProject = await project_service_1.projectsService.createProject({
        userId,
        businessId,
    ...
    const expectedBudget = Number(brandingService.unit_price) + Number(devDayService.unit_price) * 5;
    ...
    console.log('\n=== 46) project tasks & milestones progress ===');
    await project_service_1.projectsService.updateProject(createdProject.project.id, userId, { progressAutoMode: 'tasks' });
    const taskA = await project_task_service_1.projectTaskService.addTask({
        userId,
        projectId: createdProject.project.id,
        title: 'Plan workshop',
    });
    const taskB = await project_task_service_1.projectTaskService.addTask({
        userId,
        projectId: createdProject.project.id,
        title: 'Build homepage',
    });
    await project_task_service_1.projectTaskService.updateTask(taskA.id, userId, { status: 'done' });
    const taskProgress = await project_service_1.projectsService.computeProjectProgress(createdProject.project.id, userId);
    console.log('✅ Task-based progress:', { value: taskProgress.value });
    const milestone1 = await project_milestone_service_1.projectMilestoneService.addMilestone({
        userId,
        projectId: createdProject.project.id,
        name: 'Discovery',
        weightPct: 50,
    });
    const milestone2 = await project_milestone_service_1.projectMilestoneService.addMilestone({
        userId,
        projectId: createdProject.project.id,
        name: 'Delivery',
        weightPct: 50,
    });
    await project_milestone_service_1.projectMilestoneService.updateMilestone(milestone1.id, userId, { status: 'completed' });
    await project_service_1.projectsService.updateProject(createdProject.project.id, userId, { progressAutoMode: 'milestones' });
    const milestoneProgress = await project_service_1.projectsService.computeProjectProgress(createdProject.project.id, userId);
    console.log('✅ Milestone-based progress:', { value: milestoneProgress.value });
    // --- 47) project financials ---
    console.log('\n=== 47) project financials ===');
    await project_service_1.projectsService.updateProject(createdProject.project.id, userId, { progressAutoMode: 'financial' });
    await transaction_service_1.transactionService.createBusinessTransaction({
        userId,
        businessId,
        accountId: businessAccountId,
        date: new Date(),
    ...
    const projectFinancials = await project_service_1.projectsService.computeProjectFinancials(createdProject.project.id, userId);
    const financialProgress = await project_service_1.projectsService.computeProjectProgress(createdProject.project.id, userId);
    ...
    console.log('\n=== 48) listProjectsForUser ===');
    const listedProjects = await project_service_1.projectsService.listProjectsForUser(userId, { businessId });
    ...
    console.log('\n=== 49) getProjectWithDetails ===');
    const projectWithDetails = await project_service_1.projectsService.getProjectWithDetails(createdProject.project.id, userId);

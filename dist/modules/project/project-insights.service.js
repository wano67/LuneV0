...
exports.projectInsightsService = exports.ProjectInsightsService = void 0;
...
class ProjectInsightsService {
    ...
    async getOverview(userIdInput, projectIdInput) {
        ...
        const allHaveEstimates = tasks.length > 0 && tasks.every((t) => t.estimated_hours != null && decimalToNumber(t.estimated_hours) > 0);
        let progressPct = 0;
        let weighted = false;
        if (allHaveEstimates) {
            const weightedSum = tasks.reduce((sum, t) => sum + (decimalToNumber(t.estimated_hours) || 0) * (t.progress_pct ?? 0), 0);
        ...
        for (const task of tasks) {
            ...
            if (task.dependencies && task.dependencies.length > 0) {
                ...
                if (hasBlocking)
                    blockingTasks.push(task.id.toString());
            }
        ...
        const startDates = tasks.map((t) => t.start_date).filter((d) => !!d);
        const dueDates = tasks.map((t) => t.due_date).filter((d) => !!d);
...
exports.projectInsightsService = new ProjectInsightsService(prisma_1.prisma);

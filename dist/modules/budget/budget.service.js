...
exports.budgetService = exports.BudgetService = exports.PersonalBudgetNotFoundError = exports.InvalidBudgetInputError = exports.BudgetOwnershipError = exports.BudgetLineNotFoundError = exports.BudgetNotFoundError = void 0;
...
class BudgetLineNotFoundError extends Error {
    constructor(message = 'Budget line not found') {
        super(message);
        this.name = 'BudgetLineNotFoundError';
    }
}
exports.BudgetLineNotFoundError = BudgetLineNotFoundError;
...
class InvalidBudgetInputError extends Error {
    constructor(message) {
        super(message);
        this.name = 'InvalidBudgetInputError';
    }
}
exports.InvalidBudgetInputError = InvalidBudgetInputError;
class PersonalBudgetNotFoundError extends Error {
    constructor(message = 'Personal budget not found for current period') {
        super(message);
        this.name = 'PersonalBudgetNotFoundError';
    }
}
exports.PersonalBudgetNotFoundError = PersonalBudgetNotFoundError;
...
function validatePriority(priority) {
    if (priority == null)
        return;
    const allowed = ['essential', 'comfort', 'nice_to_have'];
    if (!allowed.includes(priority)) {
        throw new InvalidBudgetInputError('Invalid budget line priority');
    }
}
function validateAlertThreshold(alert) {
    if (alert == null)
        return;
...
function validateSpendingLimit(limit) {
    if (limit == null)
        return;
...
class BudgetService {
    ...
    async createPersonalBudget(input) {
        ...
        const budget = await this.prismaClient.budgets.create({
            data: {
                ...
                include_accounts: input.includeAccounts ??
    ...
    async createBusinessBudget(input) {
        const userId = (0, ids_1.normalizeUserId)(input.userId);
    ...
    async getBudgetWithLinesForUser(budgetIdInput, userIdInput) {
        const budgetId = (0, ids_1.normalizeBudgetId)(budgetIdInput);
    ...
    async listBudgetsForUser(userIdInput, filters) {
        const userId = (0, ids_1.normalizeUserId)(userIdInput);
        const where = {
            user_id: userId,
        };
    ...
    async updateBudget(budgetIdInput, userIdInput, input) {
        const budgetId = (0, ids_1.normalizeBudgetId)(budgetIdInput);
        const userId = (0, ids_1.normalizeUserId)(userIdInput);
        ...
        const periodType = (input.periodType ?? existing.period_type);
        validatePeriod({
            periodType,
        ...
        if (input.name !== undefined)
            data.name = normalizeBudgetName(input.name);
        if (input.periodType !== undefined)
            data.period_type = input.periodType;
        ...
        if (input.versionNo !== undefined)
            data.version_no = input.versionNo;
        if (input.status !== undefined)
            data.status = input.status;
        if (input.totalSpendingLimit !== undefined)
            data.total_spending_limit = input.totalSpendingLimit;
        if (input.includeAccounts !== undefined)
            data.include_accounts = input.includeAccounts;
    ...
    async archiveBudget(budgetIdInput, userIdInput) {
        return this.updateBudget(budgetIdInput, userIdInput, { status: 'archived' });
    }
    async reactivateBudget(budgetIdInput, userIdInput) {
        return this.updateBudget(budgetIdInput, userIdInput, { status: 'active' });
    }
    async addBudgetLine(input) {
        const budgetId = (0, ids_1.normalizeBudgetId)(input.budgetId);
        const userId = (0, ids_1.normalizeUserId)(input.userId);
        const categoryId = (0, ids_1.normalizeCategoryId)(input.categoryId);
        const categoryGroupId = input.categoryGroupId !== undefined && input.categoryGroupId !== null
            ? (0, ids_1.normalizeCategoryId)(input.categoryGroupId)
            : null;
    ...
    async updateBudgetLine(lineIdInput, userIdInput, input) {
        const lineId = (0, ids_1.normalizeBudgetLineId)(lineIdInput);
        const userId = (0, ids_1.normalizeUserId)(userIdInput);
    ...
    async deleteBudgetLineForUser(lineIdInput, userIdInput) {
        const lineId = (0, ids_1.normalizeBudgetLineId)(lineIdInput);
        const userId = (0, ids_1.normalizeUserId)(userIdInput);
    ...
    async computeBudgetExecutionForUser(budgetIdInput, userIdInput) {
        const budgetId = (0, ids_1.normalizeBudgetId)(budgetIdInput);
        const userId = (0, ids_1.normalizeUserId)(userIdInput);
        ...
        const linesWithExecution = lines.map((line) => {
    ...
    async computeCurrentPersonalBudgetOverview(userIdInput, options) {
        const userId = (0, ids_1.normalizeUserId)(userIdInput);
        ...
        const budget = await this.prismaClient.budgets.findFirst({
            where: {
                ...
                month,
                status: 'active',
                include_accounts: { path: ['scope'], equals: 'personal' },
            },
            select: budgetSelect,
        });
        if (!budget) {
    ...
    extractBusinessScope(budget) {
        if (!budget.include_accounts)
            return null;
        try {
            const parsed = typeof budget.include_accounts === 'string'
                ? JSON.parse(budget.include_accounts)
                : budget.include_accounts;
            if (parsed?.scope === 'personal')
                return 'personal';
            if (parsed?.businessId)
                return (0, ids_1.normalizeBusinessId)(BigInt(parsed.businessId));
        }
        catch {
            return null;
        }
    ...
    async assertBudgetOwnedByUser(budgetId, userId) {
        const budget = await this.prismaClient.budgets.findUnique({
            where: { id: budgetId },
            select: budgetSelect,
        });
    ...
    async assertBudgetLineOwnedByUser(lineId, userId) {
...
exports.budgetService = new BudgetService(prisma_1.prisma);

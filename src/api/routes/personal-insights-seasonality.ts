...
import { personalSeasonalitySchema } from '@/api/schemas/personal-insights-seasonality';
...
export async function registerPersonalInsightsSeasonalityRoutes(app: FastifyInstance) {
  ...
  server.route({
    ...
    schema: {
      ...
      response: {
        200: z.object({ data: personalSeasonalitySchema }),
      },
    },
    ...
    async handler(request, reply) {
      ...
      const data = await personalInsightsSeasonalityService.getSeasonality({ userId, months });

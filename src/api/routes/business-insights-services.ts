...
import { businessTopServicesSchema } from '@/api/schemas/business-insights-services';
import { businessInsightsServicesService } from '@/modules/business/business-insights-services.service';
...
export async function registerBusinessInsightsServicesRoutes(app: FastifyInstance) {
  ...
  server.route({
    ...
    schema: {
      ...
      response: {
        200: z.object({ data: businessTopServicesSchema }),
      },
    },
    ...
    async handler(request, reply) {
      ...
      const data = await businessInsightsServicesService.getTopServices({
        userId,
        businessId,
        from: fromDate,
        to: toDate,
